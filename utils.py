...
# Pure Python logic (no library dependencies) in the 'explicit' functions below, 
#   following https://github.com/msgpack/msgpack/blob/master/spec.md
# Algorand's serialization fmt additionally omits zero uints and empty arrays

def process(s):
  return bytes(ast.literal_eval(",".join(s.split())))

def serialize_state_proof_message(
  block_headers_commitment: bytes, # 32 bytes
  voters_commitment: bytes,
  ln_proven_weight: int, # uint64
  first_attested_round: int, # uint64
  last_attested_round: int # uint64
):
  spm = {}
  spm['b'] = block_headers_commitment
  spm['v'] = voters_commitment
  if ln_proven_weight:
    spm['P'] = ln_proven_weight
  if first_attested_round:
    spm['f'] = first_attested_round
  if last_attested_round:
    spm['l'] = last_attested_round

  return msgpack.packb(spm)
...
def serialize_light_block_header(
  ...
  if round_number:
    lbh['r'] = round_number
  lbh['tc'] = tx_commitment

  return msgpack.packb(lbh)

def serialize_light_block_header_explicit(
  seed: bytes,
  genesis_hash: bytes,
  round_number: int,
  tx_commitment: bytes
  ...
  if round_number != 0:
    lbh += bytes.fromhex('a1')
    lbh += 'r'.encode()
    lbh += msgpack_int_serialize(round_number)
...
def msgpack_int_serialize(x: int) -> bytes:
  assert(x > 0)
...
if __name__ == '__main__':
  ...
  lbh_2 = serialize_light_block_header_explicit(
    seed,
    genesis_hash,
    round_number,
    tx_commitment
